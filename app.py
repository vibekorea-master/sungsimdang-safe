...
# 1. 페이지 설정 및 세션 상태 관리 (보안 및 게이트 기능의 기초)
...
# ---------------------------------------------------------
# [GATE] Safety Gate (면책 조항 차단막 - 기존 기능 완벽 유지)
# ---------------------------------------------------------
...
# ---------------------------------------------------------
    ...
    st.sidebar.markdown("**How to use:** Select your allergies. Breads containing those items will be automatically hidden.")
    ...
    # 4. 전체 29종 상세 데이터 통합 (Source 1-157 완벽 반영) 
    bread_data = [
        ...
        {"id": 17, "name": "French Pie", "ko": "후렌치파이", "price": "2,000", "origin": "Butter {Processed butter (Imported: New Zealand, Netherlands, Australia, etc.); Vegetable oil [Palm oil (Malaysia), Palm stearin (Malaysia)]}; Strong wheat flour (USA / Canada); Raspberry jam (Denmark); Strawberry jam (Strawberry: Domestic)", "allergens": ["Egg", "Wheat", "Milk"]},
        ...
        {"id": 26, "name": "Pie Manju Set", "ko": "파이만주세트", "price": "9,600", "origin": "Whole red bean paste [Red bean (China), Refined salt (Domestic)]; Strong wheat flour (USA / Canada); Almond powder (USA); Margarine {Processed fats and oils [Palm esterified oil (Malaysia), Palm kernel esterified oil (Malaysia)]; Prepared edible fat (New Zealand, Australia, Malaysia, etc.)}", "allergens": ["Egg", "Soybean", "Wheat", "Milk", "Walnut"]},
        ...
        {"id": 28, "name": "Twiso-guma", "ko": "튀소구마", "price": "1,700", "origin": "Sweet potato (Domestic); Processed cereal product [Wheat flour (Wheat: USA, Canada); Blended skim milk powder (Skim milk powder — Imported: USA, France, Belgium, etc.)]; Cake flour (USA)", "allergens": ["Egg", "Soybean", "Wheat", "Sulfites", "Milk"]},
        {"id": 29, "name": "Twigim Soboro", "ko": "튀김소보로", "price": "1,700", "origin": "Whole red bean paste [Red bean (China), Refined salt (Domestic)]; Processed cereal product [Wheat flour (Wheat: USA, Canada); Blended skim milk powder (Skim milk powder — Imported: USA, France, Belgium, etc.)]; Cake flour (USA)", "allergens": ["Egg", "Soybean", "Wheat", "Sulfites", "Milk"]}
    ]
    ...
    # 동적 필터 상태 표시 (직관성 강화)
    if avoid:
        st.success(f"✅ **Safe Results:** Showing **{len(filtered)}** items without: **{', '.join(avoid)}**")
    else:
        st.info(f"ℹ️ Showing all **{len(filtered)}** items. Use the sidebar to filter allergens.")

    # 7. 프리미엄 카드 그리드 (상세 데이터 팝업 모달 통합)
    ...
    <style>
        ...
        .modal-content {{ background: white; padding: 30px; border-radius: 2.5rem; max-width: 600px; width: 100%; position: relative; max-height: 85vh; overflow-y: auto; box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.5); }}
    </style>

    <div id="modal" class="modal-overlay" onclick="closeModal()">
        <div class="modal-content" onclick="event.stopPropagation()">
            <button class="absolute top-5 right-7 text-3xl font-bold text-gray-400 hover:text-gray-600" onclick="closeModal()">×</button>
            <h2 id="m-title" class="text-2xl font-bold text-[#78350f] mb-1"></h2>
            <p id="m-ko" class="text-sm text-orange-700 font-bold mb-5"></p>
            <hr class="border-orange-100 mb-5">
            <div class="space-y-5 text-[13px] text-gray-700">
                <div class="flex items-center"><span class="w-24 font-bold text-orange-900 uppercase tracking-tighter">💰 Price:</span> <span id="m-price" class="font-bold"></span> KRW</div>
                <div><p class="font-bold text-orange-900 uppercase tracking-tighter mb-1">🌾 Detailed Origin Info:</p> <p id="m-origin" class="bg-gray-50 p-3 rounded-xl leading-relaxed text-gray-600"></p></div>
                <div><p class="font-bold text-orange-900 uppercase tracking-tighter mb-1">🚫 Allergens Found:</p> <p id="m-allergens" class="text-gray-600"></p></div>
                <div class="bg-orange-50 p-4 rounded-2xl border border-orange-100">
                    <p class="text-[10px] font-bold text-orange-800 mb-1 uppercase tracking-widest">🏭 Shared Facility Notice:</p>
                    <p class="text-[10px] text-orange-700 leading-normal italic">"{FACILITY_NOTICE}"</p>
                </div>
            </div>
        </div>
    </div>
    ...
    # 8. 하단 전체 데이터 시트 (체류 시간 극대화 장치) 
    st.markdown("---")
    st.markdown("### 📊 Comprehensive Bread Data Sheet (Full 29 Items)")
    st.caption("Detailed origin and allergen breakdown based on the latest official English document.")
    st.table([{"ID": b["id"], "Name": f"{b['name']} ({b['ko']})", "Price": f"{b['price']}W", "Origin": b["origin"], "Allergens": ", ".join(b["allergens"])} for b in bread_data])
